...
// 插件使用说明
export const usage = `
<h2><a href="https://www.npmjs.com/package/koishi-plugin-gouqi-impart-pro-custom" target="_blank">点我查看完整README</a></h2>
<hr>
<table>
<thead>
<tr>
...
<p>举例说明：<br>
每次锻炼成功后，武器长度的增长范围。<br>
以默认值 <code>[10, 45]</code> 为例，表示成功锻炼后武器长度增长的基数为 10 厘米，同时允许有 ±45% 的浮动：</p>
<ul>
<li><strong>最大值</strong>: 10 + 10 × 0.45 = 14.5 厘米</li>
<li><strong>最小值</strong>: 10 - 10 × 0.45 = 5.5 厘米</li>
</ul>
<p>因此，锻炼成功时，武器的长度会在 5.5 厘米到 14.5 厘米之间随机增长。</p>
<hr>
...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    commandList: Schema.object({
      ...
      command3: Schema.string().default("锻炼").description("开导 令名称"),
      command4: Schema.string().default("决斗").description("决斗 指令名称"),
      command5: Schema.string().default("重开").description("重开 指令名称"),
      command6: Schema.string().default("注入排行榜").description("注入排行榜 指令名称"),
      command7: Schema.string().default("武器排行榜").description("武器排行榜 指令名称"),
      command8: Schema.string().default("看看武器").description("看看武器 指令名称"),
      command9: Schema.string().default("锁武器").description("锁武器 指令名称"),
    }).collapse().description("指令名称列表<br>自定义指令名称"),
  }).description('指令名称设置'),

  Schema.object({
    defaultLength: Schema.tuple([Number, Number]).description("【初始生成】的武器长度（cm）<br>右侧代表最大的偏差百分比（%）（默认在 18 ± 45%）").default([18, 45]),
    ...
    exerciseWinGrowthRange: Schema.tuple([Number, Number]).description("【锻炼成功】增长的武器长度（cm）<br>右侧代表最大的偏差百分比（%）（默认在 10 ± 45%）").default([10, 45]),
    exerciseLossReductionRange: Schema.tuple([Number, Number]).description("【锻炼失败】减少的武器长度（cm）<br>右侧代表最大的偏差百分比（%）（默认在 12 ± 45%）").default([12, 45]),
    exerciseCooldownTime: Schema.number().min(0).max(86400).step(1).default(5).description("【锻炼武器】间隔休息时间（秒）"),
  }).description('武器设置'),
